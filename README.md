...
현재 고객·가족·보험계약·계약별 보장·고객별 상담과 사용자 보장 기준 관리가 동작합니다. Tauri 데스크톱 앱에서 각 원본을 생성·수정·soft delete하고, 고객·가족 월보험료와 고객 카테고리별 보장 합계·부족/적정/과다를 확인할 수 있습니다. 데이터는 앱 전용 SQLite 파일에 저장됩니다. Dashboard·달력과 데이터 관리는 후속 실행 계획에서 순차 구현합니다.
...
네이티브 앱의 고객·가족·보험계약·보장·Benchmark 판정·상담 생성·수정·합계·soft delete와 프로세스 재시작 후 SQLite 지속성만 다시 확인하려면 다음 명령을 사용합니다.
