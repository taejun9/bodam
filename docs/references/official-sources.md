...
| PowerShell FileSystem provider | https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_filesystem_provider?view=powershell-7.6 | provider가 `FileInfo`/`DirectoryInfo`에 extra property를 추가 | provider result와 raw parent object 구분 |
| .NET DirectoryInfo.Parent | https://learn.microsoft.com/en-us/dotnet/api/system.io.directoryinfo.parent?view=net-10.0 | parent는 `DirectoryInfo` 또는 root의 null 반환 | plugin ancestor type·reparse 검사 |
| PowerShell operator precedence | https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_operator_precedence?view=powershell-7.6 | comma가 `+`보다 먼저 평가되고 괄호로 순서 override | LF/CRLF/CR scalar fixture 결속 |
...
- FileSystem provider 결과의 extended property를 `.Parent`가 반환한 raw `DirectoryInfo`에 가정하지 않는다. Plugin ancestor는 CLR type과 reparse attribute로 검사하고, comma보다 낮은 `+` precedence가 newline fixture를 분할하지 않도록 각 concatenation을 괄호로 묶어 exact 세 scalar를 요구한다.
