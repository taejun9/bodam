...
같은 Customer와 같은 `consultedAt`의 여러 행을 허용하고 자동 병합하지 않는다. 기본 목록은 활성 Customer의 활성 Consultation만 `consultedAt DESC, id ASC`로 정렬한다. 생성 뒤 Customer를 다른 ID로 옮기지 않는다. Calendar와 Dashboard는 원본을 복제하지 않고 요청 시 read model로 참조한다.

### Schedule

Schedule은 `id`, 선택 `customerId`, 필수 `title`·`scheduledOn`, 선택 `scheduledTime`·`memo`, `isCompleted`, timestamps와 `deletedAt`을 소유한다. 제목은 trim 뒤 1–200 Unicode scalar, 메모는 빈 값을 null로 정규화하고 최대 4,000자다. 날짜는 실제 `YYYY-MM-DD`이며 Calendar에서 계속 접근 가능한 `9998-12-31` 이하이고, 시간은 timezone offset 없는 분 정밀도 local `HH:mm`이다.

활성 Customer만 선택 연결할 수 있고 수정 시 연결 변경·해제를 허용한다. 연결 없는 일정도 허용하며 동일 제목·날짜·시간을 자동 병합하지 않는다. 완료 일정은 기본 Calendar에서 숨기지 않고 완료 상태로 표시한다. 삭제는 `deletedAt` soft delete이고 현재 복원 UI는 없다.
...
- Schedule의 nullable Customer FK는 hard delete `RESTRICT`, key update `CASCADE`다. 활성 Customer만 Schedule에 연결할 수 있다.
...
- Schedule을 soft delete하면 Customer와 다른 업무 원본을 수정하지 않는다. Customer를 soft delete하면 linked Schedule 원본은 유지하되 기본 목록·Calendar에서 숨기고 unlinked Schedule은 유지한다.
...
- Schedule 날짜와 선택 시간은 local date-only·wall time으로 저장하며 timezone 변경으로 이동시키지 않는다.
